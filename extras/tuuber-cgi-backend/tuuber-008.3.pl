...
use constant progamVersion => "0.8.3";
...
sub person_GET {
  ...
  my $pID=shift;
  my $q="SELECT pID, first, family, phone, email, spaces, driver FROM persons WHERE pID=?;";
  my $qh=$dbh->prepare($q);
  $qh->execute($pID);
...
#   my $q="SELECT * FROM routes WHERE rID=?;";
    my $q="SELECT routes.*, CONCAT(persons.first,' ', persons.family) AS personName FROM routes JOIN persons ON persons.pID=routes.pID WHERE rID=?;";
  my $qh=$dbh->prepare($q);
  $qh->execute($rID);
  my $result=$qh->fetchrow_hashref();
  my $json=  "{ ";
...
sub pools {
  ...
  if (uc $action eq POST) {
    ...
    $q="UPDATE pools SET driver_pID=?, navigator_pID=?, rID=? WHERE pool_ID=?;";
    my $qh=$dbh->prepare($q);
    $result=$qh->execute($driver_pID, $navigator_pID, $rID, $pool_ID);
  }
  elsif (uc $action eq GET) {
    $q="SELECT driver_pID, navigator_pID, rID FROM pools WHERE pool_ID=?;";
    my $qh=$dbh->prepare($q);
    $qh->execute($pool_ID);
    ...
    # do non-JSON dump  
    $q="SELECT pool_ID, driver_pID, navigator_pID, rID FROM pools;";
    my $qh=$dbh->prepare($q);
    $qh->execute();
    my $file;
    if ($showHtml) {
...
sub town_GET {
  ...
  my $tID=shift;
  my $q="SELECT tId, name, CONCAT(barony,', ', civilParish,', ', poorLawUnion) AS location FROM towns WHERE tID=?;";
  my $qh=$dbh->prepare($q);
  $qh->execute($tID);
  my $result=$qh->fetchrow_hashref();
  my $json=  "{ ";
...
sub chats {
  ...
  if (uc $action eq POST) {
    ...
    $q="SELECT my_pID, other_pID, text, status FROM chats WHERE line_ID=?;";
    my $qh=$dbh->prepare($q);
    $qh->execute($line_ID);
    $result=$qh->fetchrow_hashref();
...
sub loginCheck {
    ...
    my $query="SELECT password from persons WHERE email = ?";
    my $qh = $dbh->prepare($query); # prepare select query
