...
if ( uc $cmd eq version ) {
    ...
    print messageCheck($cgi->param('ver'));
    exit;
}
elsif ( uc $cmd eq msg ) {
...
elsif ( uc $cmd eq verify ) {
    verifyEmail($cgi->param('email'), $cgi->param('string'),$cgi->param('pID'));
    exit;
}
...
sub sendEmail{
  # Send email to applicant, course manager, ?(admin or tech)
  #
  # Expect: application code, retrieve everything else from DB
  # 
  my $email=shift;
  my $verificationString=shift;
  my $pID=shift;
  my $sendmail =   "| /usr/sbin/sendmail -t"; 
  my $subject = "Subject: túber sign-up\n";
  my $from = "From: tuber\@fachtnaroe.net\n";
  my $contenttype = "Content-Type: text/html; charset=utf-8\n";
  my $to = "To: $email\n";
  my $cc = "CC: fachtna.roe\@tcfe.ie\n";
  my $message="\n\nHi!\nPlease click <a href=\"https://fachtnaroe.net/tuuber?cmd=verify&email=$email&string=$verificationString&pID=$pID\">here to validate your túber account email</a> address.\n\n";
...
sub persons () {
    ...
    if ( uc $action eq POST ) {
        ...
        my $verificationString=randomVerificationString();
        if ($password ne "") {
        $q =
          "INSERT INTO persons (first, family, phone, email, password, emailVerified) VALUES (?, ?, ?, ?, ?, ?);";
        }
        else {
        $q =
          "INSERT INTO persons (first, family, phone, email, emailVerified) VALUES (?, ?, ?, ?, ?);";
        }
        my $qh = $dbh->prepare($q);
        if ($password ne "") {
          $result = $qh->execute( $first, $family, $phone, $email, $password, $verificationString )
        }
        else {
          $result = $qh->execute( $first, $family, $phone, $email, $verificationString );
        }
#         dbg("Created person with pID $result");
        my $json = "{${quoteJSON}result${quoteJSON}:${quoteJSON}";
        if ( $result > 0 ) {
            $json .= "OK";
            sendEmail($email, $verificationString, $qh->{mysql_insertid});
        }
        else {
...
sub pool_GET {
    ...
    my $pID     = shift;
    my ($q, $qh, $result, $json);
    $q="SELECT pool_ID, IF(driver_pID=?, (SELECT CONCAT(first,' ',family) as realName FROM persons WHERE persons.pID=pools.navigator_pID), (SELECT CONCAT(first,' ',family) as realName FROM persons WHERE persons.pID=pools.driver_pID) ) AS realName, rID, link_ID FROM pools WHERE pool_ID=?;";
#     $q       = "SELECT * FROM pools JOIN persons ON persons.pID=WHERE pool_ID=?;";
    $qh      = $dbh->prepare($q);
    $qh->execute($pID, $pool_ID);
    $result = $qh->fetchrow_hashref();
    $json   = "{";
    foreach ( keys %$result ) {
#         $$result{$_} = deApostrophisise( $$result{$_} );
...
sub pools_ID_list {
    ...
    my $pID = shift;
    my $status=shift;
    my $q       = "SELECT pool_ID FROM pools WHERE (status=?) AND ((driver_pID=?) OR (navigator_pID=?));";
    my $qh      = $dbh->prepare($q);
    $qh->execute($status, $pID, $pID);
    my $result = $qh->fetchall_arrayref();
    # Cast address as array
    return @{$result};
}
...
sub pools {
    ...
    $q =
        ...
        $q="SELECT pool_ID FROM pools WHERE (driver_pID=?) AND (navigator_pID=?) AND (rID=?) AND (link_ID=?)";
        $qh = $dbh->prepare($q);
        $result = $qh->execute( $driver, $navigator, $route, $link_ID );
        my $current_pool_ID_number=$qh->fetchrow();
        my ($pool_ID, $pool_creation_status);
        if ($current_pool_ID_number >= 1) {
            $q =
          "UPDATE pools SET status='open' WHERE pool_ID=?";
          #(driver_pID=?) AND (navigator_pID=?) AND (rID=?)  AND (link_ID=?)";
          $qh = $dbh->prepare($q);
          $result = $qh->execute( $current_pool_ID_number );
          $pool_ID = $current_pool_ID_number; #$dbh->{mysql_insertid};
          $number=$current_pool_ID_number;
          $pool_creation_status="existing";
        }
        else {
          $q =
        "INSERT INTO pools (driver_pID, navigator_pID, rID, status, link_ID, openedBy) VALUES (?, ?, ?, 'init', ?, ?);";
          $qh = $dbh->prepare($q);
          $result = $qh->execute( $driver, $navigator, $route, $link_ID, $pID );
          $pool_ID = $dbh->{mysql_insertid};
          $number=$pool_ID;
          $pool_creation_status="new";
        }
        ...
        print "{${quoteJSON}result${quoteJSON}:${quoteJSON}$result${quoteJSON},${quoteJSON}pool_ID${quoteJSON}:${quoteJSON}$number${quoteJSON},${quoteJSON}status${quoteJSON}:${quoteJSON}$pool_creation_status${quoteJSON}}";
    }
    elsif ( uc $action eq PUT ) {
...
#     }
        ...
        if ( !$showHtml ) {    # if not showing html, do the JSON dump
            my @list = pools_ID_list($pID, $status);
            print "{${quoteJSON}pool${quoteJSON}:[";
            foreach (@list) {
            
                my $text = pool_GET(@$_, $pID);
                print $text;
                if ( \$_ != \$list[-1] ) {
                    if ($text ne '') {
                        # if not at end of list
...
sub loginCheck {
    ...
    my $query     = "SELECT pId, password from persons WHERE (email = ?) AND (emailVerified='Y');";
    my $qh = $dbh->prepare($query);    # prepare select query
...
sub links {
    ...
    if ( uc $action eq POST ) {
        ...
        $q="SELECT COUNT(*) FROM links WHERE (first=?) AND (second=?)";
        my $qh = $dbh->prepare($q);
        $qh->execute($first, $second);
        $result = $qh->fetchrow();
        if ($result < 1) {
          $q =
        ...
        $json= "{${quoteJSON}link_ID${quoteJSON}:${quoteJSON}$link_ID${quoteJSON},${quoteJSON}result${quoteJSON}:${quoteJSON}$result${quoteJSON}}";
    }
    elsif ( uc $action eq PUT ) {
        $q =
...
sub verifyEmail{
  ...
  my $verificationString  = shift;
  my $pID=shift;
  my $q     = "SELECT emailVerified FROM persons WHERE (pID=?) AND (email = ?);";# AND (emailVerified='Y');";
  $qh = $dbh->prepare($q);    # prepare select query
  $qh->execute($pID, $userEmail);          # select now
  my $string_from_db  = $qh->fetchrow();
  if  ($string_from_db eq $verificationString) {
      $q="UPDATE persons SET emailVerified='Y' WHERE (pID=?) AND (email = ?)";
      $qh=$dbh->prepare($q);
      $qh->execute($pID, $userEmail);
      print "<h1>Thank you.</h1><h3>Your t&uacute;ber account is now active.</h3><br>(The latest version of the App will be <a href=\"https://fachtnaroe.net/apks/\">available here</a>)";
  }
  else {
    print "<h1>Sorry.</h1><h2>You did something wrong; the Internet will now reboot.</h2>The latest version of the App is <a href=\"https://fachtnaroe.net/apks/\">available here</a><br><br><small>(If the Internet remains inaccessible in your country for more than 30 minutes, phone +1-202-456-1111 and ask for Donald)</small></small><small></small>";
  }
}
...
sub messageCheck {
 ...
 my $cut_off=66;
 if ($version < $cut_off) {
    $html="{ ${quoteJSON}result${quoteJSON}:${quoteJSON}Yes${quoteJSON},${quoteJSON}detail${quoteJSON}:${quoteJSON}New version available${quoteJSON},${quoteJSON}version_is${quoteJSON}:${quoteJSON}$cut_off${quoteJSON}}";
 }
 else {
    $html="{ ${quoteJSON}result${quoteJSON}:${quoteJSON}OK${quoteJSON}}";
 }
 return $html;
}
...
sub messageForAndroid {
  ...
  my $width=shift;
  my $height= shift;
#   my ($version, $width, $height)= @{shift};
  my $fontsize=$height/4;
  my $html=<<__MSG_END;
...
<div id="msg">
<a href="https://fachtnaroe.net/apks/CoverSheetGenerator_013.apk"><b>There's a newer version available.</b><br><br>You have version 0.0$version.</a>
</div>
__MSG_END
  return $html;
}
...
# version 32: added init/open code for SQL pool data, with better JSON response
