...
sub matches_list {
    ...
    my $rID = shift;
    my $iam= $cgi->param('iam');
    my $q="SELECT routes.rID FROM routes JOIN persons ON routes.pID=persons.pID WHERE (origin LIKE (SELECT origin FROM routes WHERE rID=?) AND destination LIKE (SELECT destination FROM routes WHERE rID=?)) AND rID != ? AND routes.pID <> ?";
    my $qh  = $dbh->prepare($q);
    $qh->execute($rID, $rID, $rID, $iam);
    my @list;    
