...
use constant progamVersion => "0.8.8";
...
sub routes {
    ...
    if ( uc $action eq POST ) {
        $q =
"INSERT INTO routes (origin, destination, mon, tues, weds, thurs, fri, pID, driver) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);";
        my $qh = $dbh->prepare($q);
        $result = $qh->execute(
            $origin, $destination, $mon, $tues, $weds,
            $thurs,  $fri,         $pID, $driver
        );
        my $json = "{'rID':'" . $qh->{mysql_insertid} . "'}";
        print $json;
    }
    elsif ( uc $action eq PUT ) {
        $q =
"UPDATE routes SET origin=?, destination=?, mon=?, tues=?, weds=?, thurs=?, fri=?, pID=?, driver=? WHERE rID=?;";
        my $qh = $dbh->prepare($q);
        $result = $qh->execute(
            $origin, $destination, $mon, $tues,   $weds,
            $thurs,  $fri,         $pID, $driver, $rID
        );
        $result = "OK" if ( $result == 1 );
        my $json = "{'result':'" . $result . "'}";
        print $json;
