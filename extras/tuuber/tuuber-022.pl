...
#          print "[$who][$line_ID][$q]";
# $q =    
#          "SELECT chats.line_ID FROM chats JOIN persons ON persons.pID=chats.$who WHERE line_ID=? ORDER BY chats.line_ID ASC";
    my $qh      = $dbh->prepare($q);
    $qh->execute($line_ID);
    my $result = $qh->fetchrow_hashref();
    my $json   = "{";
...
#           if ($my_pID > 0) {
#             $q = "DELETE FROM chats WHERE (((initiator_pID=?) AND (respondent_pID=?)) AND (status=?);";
#           }
#           elsif (1) {
...
sub matches_list {
    ...
    my $q="SELECT routes.rID FROM routes JOIN persons ON routes.pID=persons.pID WHERE (origin LIKE (SELECT origin FROM routes WHERE rID=?) AND destination LIKE (SELECT destination FROM routes WHERE rID=?)) AND rID != ?;";
    my $qh  = $dbh->prepare($q);
    $qh->execute($rID, $rID, $rID);
    my @list;    
